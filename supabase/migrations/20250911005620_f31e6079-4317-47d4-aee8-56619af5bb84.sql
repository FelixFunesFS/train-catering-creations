-- Data Cleanup Plan Implementation

-- Step 1: Fix Status Inconsistencies
-- Update quotes that have priced invoices to "quoted" status
UPDATE quote_requests 
SET status = 'quoted'::quote_status, 
    workflow_status = 'quoted'::quote_workflow_status,
...
UPDATE quote_requests
SET status = 'confirmed'::quote_status,
    workflow_status = 'confirmed'::quote_workflow_status, 
    last_status_change = now()
WHERE id IN (
  SELECT qr.id
  FROM quote_requests qr 
