-- Create audit table for quote request changes
CREATE TABLE public.quote_request_history (
...
-- Add calendar sync fields to quote_requests table
ALTER TABLE public.quote_requests 
ADD COLUMN calendar_event_id TEXT,
ADD COLUMN calendar_sync_status TEXT DEFAULT 'not_synced',
ADD COLUMN last_calendar_sync TIMESTAMP WITH TIME ZONE;

-- Create index for better performance on audit queries
CREATE INDEX idx_quote_request_history_quote_id ON public.quote_request_history(quote_request_id);
CREATE INDEX idx_quote_request_history_timestamp ON public.quote_request_history(change_timestamp);

-- Enable Row Level Security
ALTER TABLE public.quote_request_history ENABLE ROW LEVEL SECURITY;

-- Create RLS policy for audit table (admin access only)
CREATE POLICY "Service role can manage history" 
ON public.quote_request_history 
FOR ALL 
USING (false);
...
CREATE TRIGGER quote_request_audit_trigger
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.log_quote_request_changes();